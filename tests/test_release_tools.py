...
def test_evidence_bundle_is_normalised(tmp_path: Path) -> None:
    ...
    (run_dir / ".guided-state.json").write_text("{}\n", encoding="utf-8")
    output = tmp_path / "veritas-ai-0.2.0-evidence.tar.gz"
    build_bundle(run_dir, output, "0.2.0", Path.cwd())
    ...
    assert not any(member.name.endswith("/.guided-state.json") for member in members)


def test_evidence_bundle_rejects_unexpected_hidden_run_file(tmp_path: Path) -> None:
    run_dir = tmp_path / "run"
    _complete_run(run_dir)
    (run_dir / ".secret").write_text("not public\n", encoding="utf-8")
    with pytest.raises(ValueError, match="Unexpected hidden run file"):
        build_bundle(run_dir, tmp_path / "evidence.tar.gz", "0.2.0", Path.cwd())
...
def test_demo_cast_uses_verified_values(tmp_path: Path) -> None:
        ...
        json.dumps(
            {
                ...
                "scenario_actions": {"partial_telemetry_loss": "investigate"},
                "demonstration": {
                    "mode": "automatic_cli",
                    "telemetry_source": "docker_zeek",
                    "zeek_validated": True,
        ...
        json.dumps(
            {
                "record_type": "event",
                "event": {
                    "scenario": "partial_telemetry_loss",
    ...
    build_cast(summary, verification, output, ledger)

    records = [json.loads(line) for line in output.read_text(encoding="utf-8").splitlines()]
    assert records[0]["version"] == 2
    rendered = "".join(record[2] for record in records[1:])
    assert "5000 windows" in rendered
    assert "partial_telemetry_loss" in rendered
    assert "labels unavailable" in rendered
    assert "Ledger valid  True" in rendered
    assert "Workflow mode  automatic_cli" in rendered
    assert "Operator acknowledgement  True" in rendered
    assert "stable recovery windows  2" in rendered
...
def test_release_workflow_checks_anonymous_container_access() -> None:
    ...
    assert "&& grep -q 'linux/amd64'" in workflow
    assert "&& grep -q 'linux/arm64'" in workflow
    assert 'VERSION: "0.2.0"' in workflow
    assert "Smoke test the guided dashboard" in workflow
    assert "127.0.0.1:8501:8501" in workflow
